...
## 5. 增加字体
### hankaku.bin
hankaku.txt中每一行映射成8位二进制数字的数组，一共`16*256=4kb`，最后32个字体为空
### ? obj文件格式
## 6. 显示字符串
c语言用`0x00`或`\0`作为字符串的结尾
## 7. 显示变量值
### ? sprintf函数
## 8. 显示鼠标指针
* 生成鼠标指针像素颜色数据
...
## GDT和IDT的初始化
* 分段，多任务避免内存地址冲突
* 32位模式下 `MOV EAX, [DS:EBX]` 中 DS(8n) 代表段号，DS 16位，但只有高13位可用，所以段最多有`8*1024=8192`个段
* 表示一个段，需要8个字节 32位，其中起始地址32位，长度20位，属性12位，段最多占用 `8*8*1024=64kb`
